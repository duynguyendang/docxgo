...
func setupHeader(header domain.Header) {
	...
	run, _ := para.AddRun()
	run.AddText("go-docx v3 • Advanced Features Demo")
...
func addCoverPage(doc domain.Document) {
	...
	authorRun.AddText("Created with go-docx v3")
...
func addIntroduction(doc domain.Document) {
	...
	introRun.AddText("This document demonstrates the advanced features available in go-docx v3. It showcases sections, headers, footers, fields, and comprehensive style management.")
...
func addFeatures(doc domain.Document) {
	...
	intro.SetStyle(domain.StyleIDNormal)
	introRun, _ := intro.AddRun()
	introRun.AddText("go-docx v3 introduces several powerful capabilities:")
...
func addConclusion(doc domain.Document) {
	...
	summaryRun.AddText("This document demonstrates the complete feature set of go-docx v3. All advanced capabilities work together seamlessly to create professional documents programmatically.")
	...
	finalRun.AddText("Thank you for using go-docx v3!")
